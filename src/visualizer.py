...
from .simulation import SimulationDeadlockError
import os
import math
import random
...
class Visualizer:
    def __init__(self, graph, simulation, start_hub_name=None,
    ...
    def reset(self):
        self.simulation.zone_occupancy = {}
        for name in self.graph.zones:
            self.simulation.zone_occupancy[name] = 0
        self.simulation.zone_occupancy[self.start_hub_name] = self.nb_drones
        self.simulation.drones = []
        for i in range(self.nb_drones):
            drone = Drone(self.path, drone_id=i)
            self.simulation.drones.append(drone)
        self.last_move_time = pygame.time.get_ticks()
        self.simulation.turn_count = 0
            ...
            while running:
                ...
                for zone in self.graph.zones.values():
                    ...
                    if zone.name not in (self.start_hub_name, self.end_hub_name):
                        capacity_text = f"{self.simulation.zone_occupancy[zone.name]} ⁄ {zone.max_drones}"
